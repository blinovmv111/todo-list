...
// fetch("http://localhost:3001/lists?_expand=color", {body: JSON.stringify({user: 1})}).then(res => res.json()).then(json => {}) //получение данных по http запросу с преобразованием данных в json строку
...
function App() {
  ...
  useEffect(() => {
    axios.get("http://localhost:3000/lists?_expand=color&_embed=tasks").then(({data}) => {
    setLists(data)});
